...
export async function getClientTrainings(year: number, month: number): Promise<ClientTraining[]> {
  const searchParams = new URLSearchParams({
    year: String(year),
    month: String(month),
  });
  const response = await fetch(`/api/client-trainings?${searchParams.toString()}`, {
    cache: "no-store",
  });

  if (!response.ok) {
    const result = (await response.json().catch(() => null)) as { error?: string } | null;
    throw new Error(result?.error ?? "Не удалось загрузить отметки о тренировках.");
  }
