...
export function ChangePasswordForm() {
  ...
  return <>
    <button type="button" onClick={() => setIsOpen(true)} className="w-full max-w-full rounded-xl border border-accent/35 bg-accent-soft px-3 py-2 text-sm font-semibold text-accent-foreground transition hover:border-accent/60 sm:w-auto">Сменить пароль</button>
    {isOpen && <dialog ref={dialogRef} onClose={() => setIsOpen(false)} onCancel={(event) => { if (pending) event.preventDefault(); }} onClick={(event) => { if (event.target === dialogRef.current) close(); }} className="m-auto w-[min(92vw,30rem)] rounded-2xl border border-border bg-white p-0 text-foreground shadow-[0_30px_80px_-28px_rgba(49,35,24,0.55)] backdrop:bg-black/30 backdrop:backdrop-blur-[2px]">
      <div className="flex items-center justify-between gap-4 border-b border-border px-5 py-4"><h3 className="text-xl font-semibold">Смена пароля</h3><button type="button" onClick={close} disabled={pending} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xl leading-none text-muted transition hover:bg-surface-muted hover:text-foreground disabled:opacity-50" aria-label="Закрыть">×</button></div>
