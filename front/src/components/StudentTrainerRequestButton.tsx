...
export function StudentTrainerRemoveButton({ coachId, coachName }: { coachId: string; coachName: string }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string>();
  ...
  async function removeTrainer() {
    ...
    try {
      ...
      if (!response.ok) {
        const result = (await response.json().catch(() => null)) as { error?: string } | null;
        setError(result?.error ?? "Не удалось открепиться от тренера.");
        return;
      }
  ...
  return (
    <>
      <button type="button" onClick={openModal} className="shrink-0 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-danger transition hover:bg-danger-soft" aria-label={`Открепиться от тренера «${coachName}»`}>
        Удалить
      </button>

      <dialog ref={dialogRef} onClick={(event) => { if (event.target === dialogRef.current) closeModal(); }} className="m-auto w-[min(92vw,30rem)] rounded-2xl border border-border bg-white p-0 text-foreground shadow-[0_30px_80px_-28px_rgba(49,35,24,0.55)] backdrop:bg-black/30">
        <div className="border-b border-border px-5 py-4 sm:px-6">
          <h2 className="text-xl font-semibold">Открепиться от тренера?</h2>
          <p className="mt-1 text-sm text-muted">Тренер {coachName} будет удалён из вашего списка.</p>
        </div>
        <div className="flex flex-col gap-4 px-5 py-5 sm:px-6">
          {error && <p className={errorClass}>{error}</p>}
          <p className="text-sm leading-6 text-muted">Вы перестанете состоять в группах этого тренера и сможете позднее отправить ему новую заявку.</p>
          <div className="flex justify-end gap-3">
            <button type="button" onClick={closeModal} disabled={isSubmitting} className="rounded-xl border border-border px-4 py-2.5 text-sm font-medium hover:bg-surface-muted disabled:opacity-50">
              Отмена
            </button>
            <button type="button" onClick={() => void removeTrainer()} disabled={isSubmitting} className="rounded-xl bg-danger px-5 py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60">
              {isSubmitting ? "Удаление…" : "Открепиться"}
            </button>
          </div>
        </div>
      </dialog>
